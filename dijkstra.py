...
def dijkstra(puzzle):
    ...
    while len(fringe) > 0:
        dist, current = heapq.heappop(fringe)
        ...
        for a in actions:
            nxt = current.apply_action(a)
            if nxt.to_string() in concluded:
                continue
            prev[nxt.to_string()] = current
            distances[nxt.to_string()] = dist+1
            heapq.heappush(fringe, (dist+1, nxt))
